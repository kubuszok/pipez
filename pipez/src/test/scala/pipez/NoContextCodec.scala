...
object NoContextCodec extends NoContextCodecCompanionCompat {

  type NoContextResult[A] = Either[List[String], A]

  // Information for macros to let them now how to combine smaller instances into bigger instances
  implicit val pipeDerivation: PipeDerivation.Aux[NoContextCodec, Unit, NoContextResult] =
